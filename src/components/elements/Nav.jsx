...
const Nav = ({ theme, position }) => {
  ...
  const navList = ["Showcase", "Catalog", "Delivery", "Rewards"];
  return (
      ...
      <div className="container py-5 mx-auto">
        <div className="flex items-center">
          ...
          <div className="w-auto ">
            <ul
              className={`${
                toggle === true ? `visible opacity-100` : `invisible opacity-0`
              } fixed inset-0 flex flex-col items-center justify-center  bg-white  md:visible md:flex-row md:bg-transparent md:relative md:opacity-100 md:flex md:items-center`}
              id="menu"
            >
              {navList.map((list, index) => {
                return (
                  <li key={index} className="py-6 mx-3 md:py-0">
                    <a
                      href="#"
            ...
            <ul className="flex items-center">
              <li className="block ml-6 md:hidden">
                <button
                  ...
                  className={` ${
                    toggle === true ? `absolute top-0 right-0` : `relative`
                  } z-50 flex items-center justify-center w-8 h-8 focus:outline-none `}
                >
                  <svg
                    width="18"
                    height="17"
              ...
              <li className="ml-6">
                <Link
                  id="header-cart"
                  className={`flex cart items-center justify-center w-8 h-8 ${
                    theme === `light`
                      ? `text-black md:text-white`
                      : `text-black`
                  } ${cartChanged ? `animate-bounce` : ``} ${cart.length > 0 ? `cart-filled` : ``}`}
                  to={"/cart"}
                >
                  <svg
                  ...
                  >
                    ...
                    <g className="text-pink-400">
                      <circle cx='21' cy='5' r='5' className="fill-current dot"></circle>
                    </g>
                  </svg>
                </Link>
              </li>
