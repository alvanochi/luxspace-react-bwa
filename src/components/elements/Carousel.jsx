...
export default function Carousel({ children, refContainer }) {
  ...
  const fnCheckIndex = useCallback(
    (e) => {
      if (e.propertyName === "left") {
        setTimeout(() => {
          refDragHandler.current.classList.remove("transition-all");
          refDragHandler.current.classList.remove("duration-200");
        }, 200);
  ...
  const fnShiftItem = useCallback(
    (direction) => {
      ...
      refDragHandler.current.classList.add("duration-200");
