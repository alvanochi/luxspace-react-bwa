...
export const Loading = () => {
  return (
    <section className="container mx-auto">
      ...
      <div className="h-10 mb-2 rounded-full w-60 bg-slate-300"></div>
      <div className="w-32 h-6 rounded-full bg-slate-300"></div>
          </div>
          <div className="flex-1">
            <div className="slider">
              <div className="px-4 thumbnail">
                {Array(5).fill().map((_,index) => {
                  return (
                <div key={index} className="mb-4 mr-2 rounded-lg md:mr-0 w-28 h-28 bg-slate-300">
                </div>
                  )
                })}
              ...
              <div className="preview">
                <div className="w-full h-full overflow-hidden rounded-lg bg-slate-300 item">
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-32 h-6 rounded-full bg-slate-300"></div>
            ...
            {Array(5).fill().map((_,index) => {
              return (
                <div key={index} className="w-full mb-4 rounded-full h-7 bg-slate-300"></div>
              )
            })}
          </div>
        </div>
  ...
  <section className="container mx-auto">
    <div className="flex flex-wrap my-4 md:my-12">
          ...
          <div className="slider">
            <div className="thumbnail">
              {data.imgUrls.map((item,index) => {
                return (
              <div onClick={() => setSlider(item)} key={index} className="px-2">
                <div className={` item ${slider === item ? `selected bg-gray-100` : ``}`} data-img={item}>
                  <img
                    src={item}
                    alt={item}
            ...
            <div className="preview">
              <div className="h-full overflow-hidden rounded-lg item">
                <img
                  src={slider || data.imgUrls[0]}
                  alt={slider || data.imgUrls[0]}
                  className="object-cover w-full h-full rounded-lg"
                />
              </div>
            </div>
        ...
        <div className="flex-1 px-4 md:p-6">
          ...
          <p className="text-xl">Rp {data.price.toLocaleString("ID")}</p>

          <button
          onClick={() => dispatch({type: "ADD_TO_CART", payload: data})}
            className="inline-flex px-8 py-3 mt-4 text-black transition-all duration-200 bg-pink-200 rounded-full hover:bg-black hover:text-pink-400"
          >
            <svg
          ...
          <h6 className="mb-4 text-xl font-semibold">About the product</h6>
          <div
            className="mb-6 text-xl leading-7"
